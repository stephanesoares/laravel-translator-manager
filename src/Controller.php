<?php namespace StephaneSoares\TranslationManager;

use Illuminate\Http\Request;
use Illuminate\Routing\Controller as BaseController;
use StephaneSoares\TranslationManager\Models\Translation;
...
{
    /** @var \StephaneSoares\TranslationManager\Manager  */
    protected $manager;
    ...
    public function getIndex($group = null)
    {
        ...
        foreach($allTranslations as $translation){
         ...
         return view('translation-manager::index')
            ...
            ->with('editUrl', $group ? action('\StephaneSoares\TranslationManager\Controller@postEdit', [$group]) : null)
    ...
    public function postAddGroup(Request $request)
    {
        ...
        if ($group)
        {
            return redirect()->action('\StephaneSoares\TranslationManager\Controller@getView',$group);
        }
        else
        {
            return redirect()->back();
    ...
    public function postTranslateMissing(Request $request){
        ...
        if($request->has('with-translations') && $request->has('base-locale') && in_array($request->input('base-locale'),$locales) && $request->has('file') && in_array($newLocale, $locales)){
            ...
            foreach ($base_strings as $base_string) {
                ...
                app()->call(
                    'StephaneSoares\TranslationManager\Controller@postEdit',
                    [
                        'group' => $group
                    ]
                );
